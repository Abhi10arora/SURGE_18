...
int main()
{
  ...
  int nx = 128, ny = 128, x, y, t, j = 0, no_etas = 3, total_time = 2000, print_time = 200, update_print_time = 200;
  double dt = 0.5, dx = 1.0, dy = 1.0, K_conc = 3.5, K_eta = 0.625, M = 1.0, L = 1.0, w = 0.1, kx, ky, temp = 0;
  fftw_plan plan_f_conc, plan_f_g_conc, plan_b_conc, plan_b_g_conc, plan_f_eta1, plan_f_g_eta1, plan_b_eta1, plan_b_g_eta1;
  fftw_complex *conc, *g_conc, *eta1, *g_eta1;
  double rad_0 = 20;
  ...
  for(x = 0; x<nx; x++)
  {
    ...
    {
      int i = x*nx + y;
      if(pow(x-nx/2,2) + pow(y-ny/2,2) <= pow(rad_0,2))
      {
        conc[i] = 1.0;
        eta1[i] = 1.0;
      }
  ...
  for(t = 1; t<=total_time; t++)
  {
    ...
    for(x = 0; x<nx; x++)
    {
      for(y = 0; y<ny; y++)
      {
        ...
        int i = x*nx + y;
        double h_eta = 30*pow(creal(eta1[i]),2)*(pow(creal(eta1[i]),2) - 2*creal(eta1[i]) +1);
        sum_gconc = pow(creal(eta1[i]),3)*(6*pow(creal(eta1[i]),2) - 15*creal(eta1[i]) + 10);
        sum_geta = creal(eta1[i]);
        ...
        g_eta1[i] = h_eta*(g_alpha_a*pow(creal(conc[i]),3) + g_alpha_b*pow(creal(conc[i]),2) + g_alpha_c*creal(conc[i]) + g_alpha_d)
    ...
    for(x = 0; x<nx; x++)
    {
      for(y = 0; y<ny; y++)
      {
        ...
        if(t == total_time && y == ny/2)
        {
          fprintf(f1, "%d\t%lf\t%lf\n", x, creal(conc[i]), creal(eta1[i]));
        }
      }
      if(t == print_time)
      fprintf(fp, "\n");
